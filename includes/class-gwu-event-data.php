...
class GWU_Event_Data {
	...
	/**
	 * Course Type HTML snapshot from last Hostlinks page sync (_gwu_course_type_html).
	 */
	public static function get_course_type_html( ?int $page_id = null ): string {
		$page_id = $page_id ?: (int) get_the_ID();
		if ( $page_id <= 0 ) {
			return '';
		}

		$html = get_post_meta( $page_id, '_gwu_course_type_html', true );
